	...
	public String validarCadastroComMensagem(Object obj) {
		...
		if(!StringUtils.isBlank(email.getEmailDestinatario())){
			if(!email.getEmailDestinatario().contains("@")){
				return "Validação Email: Email inválido!";
			}
		}else{
			return "Validação Email: Email está vazio!";
		}
		...
		if(!StringUtils.isBlank(email.getNomePessoaDestinatario())){
			if(!email.getNomePessoaDestinatario().toLowerCase().equals("teste")){
				if(email.getNomePessoaDestinatario().length() <= 1){
					return "Validação Email: Nome do destinatário deve ser maior que 1 digito!";
				}
			}else{
				return "Validação Email: Nome do destinatário é inválido!";
			}
		}else{
			return "Validação Email: Nome do destinatário está vazio!";
		}
		}else{
			return "Validação Email: Não foram encontrados dados de Email!";
		}
		return "OK";
	}
